...
type
  TfrmRelNotaServico = class(TForm)
    QuickRep1: TQuickRep;
    DetailBand1: TQRBand;
    ...
    procedure qrlblValorPrint(sender: TObject; var Value: String);
...
var
  frmRelNotaServico: TfrmRelNotaServico;
...
procedure TfrmRelNotaServico.ImprServico;
...
begin
 ...
 begin
  VlrRetencao := VlrRetencao + ( dmEF.cdsNfsServicoVLRRETPIS.Value +
                                 dmEF.cdsNfsServicoVLRRETCOFINS.Value +
                                 dmEF.cdsNfsServicoVLRRETCSLL.Value );
  case Contador of
  1 : begin
...
function TfrmRelNotaServico.PesqEntidade( IdentID : String ) : OleVariant;
...
procedure TfrmRelNotaServico.ImprDadosTransportador;
var Cds : TClientDataSet;
begin
...
procedure TfrmRelNotaServico.ImprDadosDestinatario;
var Cds : TClientDataSet;
begin
 cds := TClientDataSet.Create( Self );
 try
...
procedure TfrmRelNotaServico.ImprDuplicNota;
var cds : TClientDataSet;
    iSql : String;
begin
...
procedure TfrmRelNotaServico.QuickRep1BeforePrint(
...
procedure TfrmRelNotaServico.QRDBText11Print(sender: TObject;
  var Value: String);
begin
 Value := Copy( Value, 1,15 );
end;

procedure TfrmRelNotaServico.qrdbtxtNFSIDPrint(sender: TObject;
  var Value: String);
begin
 Value := StrZero( dmEF.cdsNfsNFSID.Value, 6, 0 );
end;

procedure TfrmRelNotaServico.QRDBText28Print(sender: TObject;
  var Value: String);
begin
 Value := StrZero( dmEF.cdsNfsNFSID.Value, 6, 0 );
end;

procedure TfrmRelNotaServico.qrlblValorPrint(sender: TObject;
  var Value: String);
begin
 Value := Value + ' ' + FormatFloat( '##,###,##0.00', VlrRetencao );
end;

procedure TfrmRelNotaServico.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
 Action := caFree;
 frmRelNotaServico := Nil;
end;

procedure TfrmRelNotaServico.QRDBText25Print(sender: TObject;
  var Value: String);
begin
 if Value = 'E' then
    Value := '1'
...
procedure TfrmRelNotaServico.qrdbtxtDT2Print(sender: TObject;
  var Value: String);
begin
