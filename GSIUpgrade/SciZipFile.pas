...
type
  ...
  public
    ...
    procedure SaveToFileZIP( const FileName : string; Posicao : Integer );
...
procedure TZipFile.SaveToFileZIP( const FileName : string; Posicao : Integer );
var StrStream : TStringStream ;
    AFile     : TFileStream;
    Diretorio, Arquivo : String;
begin
 Arquivo   := StringReplace( filename,'/', '\', [rfReplaceAll] );
 ...
 // Diretório não processar
 if ExtractFileName( Arquivo ) = '' then
    exit;

 StrStream := TStringStream.Create( Data[Posicao] ) ;
 AFile     := TFileStream.Create( FileName,  fmCreate or fmShareDenyRead );
 try
  AFile.CopyFrom( StrStream, StrStream.Size );
...
var
  ...
  Diretorio : String;
begin
  Diretorio := StringReplace(  filename,'/', '\', [rfReplaceAll] );

  if ExtractFileName( Diretorio ) = ''  then
     begin
      if not DirectoryExists( Diretorio ) then
         CreateDir( Diretorio );
         exit;
...
begin
  ...
  try
    ...
    until ( signature = $04034B50 ) or ( signature = 0 );
    repeat
      begin
